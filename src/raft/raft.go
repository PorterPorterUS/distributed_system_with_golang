...
func (rf *Raft) BuildSnapshot(index int, snapshotData []byte, limit int) {
	...
	DPrintf("xxxxxxxxxx 服务器%d开始创建Snapshot[Size %d]，SnapshotIndex为%d, SnapshotTerm为%d, 日志长度为 %d, 日志为%+v",
	...
	DPrintf("yyyyyyyyyyyyyyyy服务器%d创建Snapshot[Size %d]完成, SnapshotIndex : %d, SnapshotTerm : %d, 日志长度为 %d, 日志为%+v",
		rf.me, rf.StateSize(), rf.SnapshotIndex, rf.SnapshotTerm, len(rf.Logs), rf.Logs)
}
...
func (rf *Raft) InstallSnapshot(args *InstallSnapshotArgs, reply *InstallSnapshotReply) {
	...
	DPrintf(">>>>>> server %d[size %d] start install the snapshot...base on index: %d",
	...
	DPrintf(">>>>>> server %d[size %d] installed the snapshot...base on index: %d",
		rf.me, rf.StateSize(), args.LastIncludeIndex)
}
